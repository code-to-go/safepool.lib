...
func Test_Hashsplit(t *testing.T) {
	...
	blocks, err := HashSplit(blake, bytes.NewBufferString(s), 13)
	...
	blocks, err = HashSplit(blake, bytes.NewBuffer(rn), 13)
	assert.NoErrorf(t, err, "Cannot split hash: %v", err)
	...
	blocks2, err := HashSplit(blake, bytes.NewBuffer(rn), 8)
	assert.NoErrorf(t, err, "Cannot split hash: %v", err)
...
func Test_HashDiff(t *testing.T) {
	...
	diffs = HashDiff([]HashBlock{a, b}, []HashBlock{b})
	assert.Len(t, diffs, 1)
